cc.Class({
    ...
    onTouchNode(x, y){ 
        if(window.nodeConnectCurrent != 0 && window.nodeCurrentTouch != 0){
            ...
            var x1 = arrPoint[index].x - 20;
            var x2 = arrPoint[index].x + 20;
            var y1 = arrPoint[index].y - 20;
            var y2 = arrPoint[index].y + 20;                                    //nếu chạm vào node thì sẽ nối kiểu khác mà ko chạy theo tay nữa
    ...
    handleConnectMuilti(){                              //xu ly set gia tri mang khi nguoi dung keo den nhieu diem khac nhau
        ...
        window.nodeConnectCurrent = 0;
        window.arrChoice[window.nodeCurrentTouch] = 0;                              //neu khong noi thi tra lai ve 0
        window.arrChoice[window.nodeConnectCurrent] = 0;
    },
